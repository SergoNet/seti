...
public class Proxy implements Runnable {
    ...
    private enum Type {READ, WRITE, DNS_READ, DNS_WRITE, AUTH_READ, AUTH_WRITE}
    ...
    private void connect(SelectionKey key) throws IOException {
        ...
        attachment.in.put(Params.CONNECTION_OK_REPLY).flip();
    ...
    private void read(SelectionKey key) throws IOException, Exception {
        var attachment = (Attachment) key.attachment();
        if (attachment == null) {
    ...
    private void scanAndReplyAuthRequest(SelectionKey key) throws Exception {
        var attachment = (Attachment) key.attachment();
        ...
        if (data[0] != Params.VERSION) {
            throw new Exception("Auth request has invalid version, only SOCKS5 is supported", 0);
        }
        ...
        for (int method_i = 0; method_i < methods_count; method_i++) {
            ...
            if (method == Params.NO_AUTH) {
                isNoAuthMethodFound = true;
                break;
            }
        }
        if (!isNoAuthMethodFound) {
            throw new Exception("Auth request has no no auth method, only no auth method is supported", len);
        }
        ...
        attachment.out.put(Params.AUTH_NO_AUTH_REPLY).flip();
        attachment.type = Type.AUTH_WRITE;
        key.interestOps(SelectionKey.OP_WRITE);
    }
    private void scanAndReplyConnectionRequest(SelectionKey key) throws Exception, IOException {
        var attachment = (Attachment) key.attachment();
        var len = attachment.in.position();
        if (len < 4) {
        ...
        var data = attachment.in.array();
        if (data[0] != Params.VERSION) {
            throw new Exception("Connection request has invalid version, only SOCKS5 is supported", 0);
        }
        if (data[1] != Params.CONNECTION_COMMAND) {
            throw new Exception("0x%02x command is not supported, only 0x01 (connect) is supported", 1);
        }
        if (data[3] == Params.ADDR_TYPE_IPV4) {
            ...
            key.interestOps(0);
        } else if (data[3] == Params.ADDR_TYPE_HOST) {
            var hostLen = data[4];
            var hostStart = 5;
            if (len < hostLen + hostStart + 2) {
                ...
                throw new Exception("invalid connection request", 4);
            }
    ...
    public static void main(String[] args) throws IOException {
        LogManager.getLogManager().readConfiguration(Proxy.class.getResourceAsStream("/logging.properties"));
        if (args.length != 1) {
