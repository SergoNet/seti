...
public class PlayersManager {
    ...
    void updatePlayer(SnakesProto.GamePlayer player) {
        ...
        synchronized (players) {
            players.put(player.getId(), PlayerWrapper.builder().player(player).lastSeen(System.currentTimeMillis()).build());
        }
    }
    ...
    void updatePlayerWithoutTouch(SnakesProto.GamePlayer player) {
        maxPlayerId = Math.max(maxPlayerId, player.getId());
        synchronized (players) {
            players.put(player.getId(), PlayerWrapper.builder().player(player)
                            .lastSeen(players.containsKey(player.getId()) ? players.get(player.getId()).getLastSeen() : System.currentTimeMillis()).build());
        }
    }
    ...
    void changeRole(int playerId, SnakesProto.NodeRole role) {
        synchronized (players) {
            ...
            if (player != null) {
                players.put(playerId, new PlayerWrapper(
                                SnakesProto.GamePlayer.newBuilder(player.getPlayer()).setRole(role).build(), player.getLastSeen())
                );
            }
        }
    }
    ...
    public Optional<SnakesProto.GamePlayer> getMaster() {
        return players.values().stream().map(PlayerWrapper::getPlayer).filter(
                player -> player.getRole() == SnakesProto.NodeRole.MASTER).findAny();
    }

    public Optional<SnakesProto.GamePlayer> getDeputy() {
        return players.values().stream().map(PlayerWrapper::getPlayer).filter(
                player -> player.getRole() == SnakesProto.NodeRole.DEPUTY).findAny();
    }

    public Optional<SnakesProto.GamePlayer> getNormal() {
        return players.values().stream().map(PlayerWrapper::getPlayer).filter(
                player -> player.getRole() == SnakesProto.NodeRole.NORMAL).findAny();
    }
    ...
    void checkDeadWorker() {
        while (true) {
            ...
            synchronized (players) {
                ...
                players.entrySet().stream().filter(e -> e.getValue().getPlayer().getId() != myId)
                        .filter(e -> currentTime - e.getValue().getLastSeen() > Config.NODE_TIMEOUT_MS)
                        .max((a, b) -> a.getValue().getPlayer().getRole() == SnakesProto.NodeRole.MASTER ? 1 : b.getValue().getPlayer().getRole() == SnakesProto.NodeRole.MASTER ? -1 : 0).ifPresent(e -> {
