...
import ru.nsu.fit.netesov.datatypes.PlayerSignature;
import ru.nsu.fit.netesov.snake.SnakeMasterController;
...
public class PlayerController {
    ...
    public PlayerController(String name, int listenPort, SnakesProto.NodeRole role) throws IOException {
        ...
        state = SnakesProto.GameState.newBuilder().setStateOrder(0).setConfig(SnakesProto.GameConfig.newBuilder().setStateDelayMs(100))
                .setPlayers(SnakesProto.GamePlayers.getDefaultInstance()).build();
            ...
            if (control.getPlayerId() == myId) {
                playersManager.getMaster().ifPresent(master -> {
                    if (master.getId() != myId) {
                        unicastManager.sendPacket(master.getIpAddress(), master.getPort(), SnakesProto.GameMessage.newBuilder().setMsgSeq(0)
                                .setSteer(SnakesProto.GameMessage.SteerMsg.newBuilder().setDirection(control.getDirection())).build());
                    }
                });
            }
        });
    ...
    public void createGame() {
        ...
        playersManager.updatePlayer(SnakesProto.GamePlayer.newBuilder().setName(name).setId(myId).setIpAddress("").setPort(listenPort).setScore(0).setRole(role).build());
        roleLock.unlock();
    }
    ...
    private void selectDeputy() {
        playersManager.getNormal().ifPresent(
                newDeputy -> {
                    ...
                    unicastManager.sendPacket(newDeputy.getIpAddress(), newDeputy.getPort(),
                            SnakesProto.GameMessage.newBuilder().setMsgSeq(0).setRoleChange(SnakesProto.GameMessage.RoleChangeMsg.newBuilder()
                                            .setReceiverRole(SnakesProto.NodeRole.DEPUTY).setSenderRole(SnakesProto.NodeRole.MASTER).build()).build()
                    );
                }
        );
    }
    ...
    private void onPlayerDeadListener(SnakesProto.GamePlayer deadPlayer) {
        ...
        if (role == SnakesProto.NodeRole.NORMAL) {
            if (deadPlayer.getRole() == SnakesProto.NodeRole.MASTER) {
                ...
                playersManager.getDeputy().ifPresent(deputy -> playersManager.changeRole(deputy.getId(), SnakesProto.NodeRole.MASTER));
            }
        }
        if (role == SnakesProto.NodeRole.DEPUTY) {
            if (deadPlayer.getRole() == SnakesProto.NodeRole.MASTER) {
                ...
                for (var player : playersManager.getPlayers()) {
                    ...
                    unicastManager.sendPacket(player.getIpAddress(), player.getPort(),
                            SnakesProto.GameMessage.newBuilder().setMsgSeq(0).setRoleChange(
                                            SnakesProto.GameMessage.RoleChangeMsg.newBuilder().setSenderRole(SnakesProto.NodeRole.MASTER)
                                                    .setReceiverRole(!hasDeputy ? SnakesProto.NodeRole.DEPUTY : SnakesProto.NodeRole.NORMAL).build())
                                    .setSenderId(myId).setReceiverId(player.getId()).build()
                    );
                    hasDeputy = true;
                }
            }
    ...
    void sendPing(SnakesProto.GamePlayer player) {
        unicastManager.sendPacket(player.getIpAddress(), player.getPort(), SnakesProto.GameMessage.newBuilder().setMsgSeq(0)
                .setPing(SnakesProto.GameMessage.PingMsg.getDefaultInstance()).build());
    }
    ...
    private void infoWorker() {
        while (!stopped.get()) {
            logger.info(String.format("%s%d (%s/%s), game players: %d (%s)", stopped.get() ? "DEAD, " : "", myId, name, role,
                            playersManager.getPlayers().size(), playersManager.getPlayers().stream()
        ...
        while (true) {
            ...
            if (role == SnakesProto.NodeRole.MASTER) {
                ...
                if (msg.getMessage().hasJoin()) {
                    ...
                    unicastManager.sendPacket(signature.getIp(), signature.getPort(), SnakesProto.GameMessage.newBuilder().setMsgSeq(0)
                                    .setAck(SnakesProto.GameMessage.AckMsg.getDefaultInstance()).setReceiverId(playerId).build());
                    if (playersManager.getDeputy().isEmpty()) {
                        playersManager.updatePlayer(SnakesProto.GamePlayer.newBuilder().setName(msg.getMessage().getJoin().getName())
                                        .setRole(SnakesProto.NodeRole.DEPUTY).setId(playerId).setIpAddress(msg.getIp()).setPort(msg.getPort()).setScore(0).build());
                        unicastManager.sendPacket(signature.getIp(), signature.getPort(), SnakesProto.GameMessage.newBuilder()
                                        .setMsgSeq(0).setRoleChange(SnakesProto.GameMessage.RoleChangeMsg.newBuilder()
                                                        .setReceiverRole(SnakesProto.NodeRole.DEPUTY).build()).build());
                    } else {
                        playersManager.updatePlayer(SnakesProto.GamePlayer.newBuilder().setName(msg.getMessage().getJoin().getName())
                                        .setRole(SnakesProto.NodeRole.NORMAL).setId(playerId).setIpAddress(msg.getIp()).setPort(msg.getPort()).setScore(0).build());
                    }
                }
            }
            if (role != SnakesProto.NodeRole.MASTER) {
    ...
    private void sendGameStateWorker() {
        while (true) {
            if (role == SnakesProto.NodeRole.MASTER) {
                var oldState = SnakesProto.GameState.newBuilder(state).setPlayers(
                                SnakesProto.GamePlayers.newBuilder().addAllPlayers(playersManager.getPlayers())).setStateOrder(0).build();
                ...
                var msg = SnakesProto.GameMessage.newBuilder().setState(
                                SnakesProto.GameMessage.StateMsg.newBuilder().setState(state).build()).setMsgSeq(0).build();
                for (var player : playersManager.getPlayers()) {
                    unicastManager.sendPacket(player.getIpAddress(), player.getPort(), msg);
                }
            }
            try {
    ...
    private void announceWorker() {
        while (true) {
            if (role == SnakesProto.NodeRole.MASTER) {
                availableGamesManager.announce(SnakesProto.GameMessage.AnnouncementMsg.newBuilder().setCanJoin(true)
                                .setConfig(SnakesProto.GameConfig.getDefaultInstance()).setPlayers(
                                        SnakesProto.GamePlayers.newBuilder().addAllPlayers(playersManager.getPlayers())).build());
            }
            try {
    ...
    private void joinGame(String ip, int port) {
        unicastManager.sendPacket(ip, port, SnakesProto.GameMessage.newBuilder().setJoin(
                SnakesProto.GameMessage.JoinMsg.newBuilder().setName(name).build()).setMsgSeq(0).build());
    }
}
