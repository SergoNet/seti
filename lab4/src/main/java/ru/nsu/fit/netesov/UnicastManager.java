...
import ru.nsu.fit.netesov.datatypes.ToSendMessageWrapper;
...
public class UnicastManager {
            ...
            try {
                ...
                if (gameMessage.hasAck()) {
                    synchronized (sentList) {
                        var elem = sentList.stream().filter(wrapper -> wrapper.getMsgSeq() == gameMessage.getMsgSeq()).findAny();
                        if (elem.isPresent()) {
                    ...
                    var ackData = GameMessage.newBuilder().setAck(GameMessage.AckMsg.getDefaultInstance()).setMsgSeq(gameMessage.getMsgSeq()).build().toByteArray();
    ...
    private void ackCheckWorker() {
        while (true) {
            ...
            synchronized (sentList) {sentList.stream().filter(wrapper -> currentTime - wrapper.getSentAt() > ACK_CHECK_MS)
                        .forEach(wrapper -> {
