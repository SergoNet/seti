package ru.nsu.fit.netesov.gui.components;

import ru.nsu.fit.netesov.AvailableGamesManager;
    ...
    public GamesList(AvailableGamesManager availableGamesManager, Consumer<MessageWithSender> onJoinListener) {
        ...
        new Thread(() -> {
            while (true) {
                ...
                setListData(copy.stream().map(messageWithSender -> String.format("%s:%d %d players",
                                messageWithSender.getIp(),
                                messageWithSender.getPort(),
                                messageWithSender.getMessage().getAnnouncement().getPlayers().getPlayersCount())).toArray(String[]::new));
            }
        }).start();
    }
}
